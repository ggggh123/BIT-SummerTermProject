...
管理端桌面界面已于 2026-09-07 精修，详见下方“桌面管理界面”和[UI 验证记录](../../docs/test/admin-ui-polish-2026-09-07.md)。本轮没有改变此处的线程、TCP 协议或数据库行为。
...
- 管理端采用深墨绿侧栏、暖白内容卡片与翡翠绿强调色，不模拟手机比例。主窗口默认 1360×860，并验证 1280×720；登录窗口默认 880×560。
- 左侧保留 8 个入口：运营总览、电桩状态、充电桩管理、充电站管理、用户管理、接口服务、请求日志、系统健康。侧栏支持键盘操作，当前选中项保持互斥。
- 运营总览显示真实查询结果中的今日／本月／累计已完成订单营收、7／30 日趋势、设备状态分布与每日明细。图表由 Qt Widgets 原生绘制，无需 Qt Charts；没有为展示补造增长率或经营数值。
- 表格为只读视图，保留行选择和原有业务按钮。设备类型、设备状态、账户状态和成功响应使用中文标签；完整原值保留在模型与悬停提示中。
...
```bash
QT_QPA_PLATFORM=offscreen ctest --test-dir /path/to/build \
  -R '^admin_' --output-on-failure
```

`admin_ui` 覆盖导航互斥与键盘、只读表格、中文状态、紧凑尺寸、7／30 日查询、空结果、坐标可访问文本和危险操作取消；`admin_login_style` 覆盖密码隐藏、必填校验、Tab 顺序和真实认证失败后重试。`admin_window_refresh` 继续检查实际业务写入与刷新兼容性。设置 `EV_ADMIN_UI_CAPTURE_DIR` 后直接执行 `tst_admin_ui`，可额外生成真实 Qt 截图；普通 CTest 不产生截图。
