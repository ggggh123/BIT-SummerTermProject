...
7. **ADS 质量表 R02/R04 检出 0**：注入各 11,200 条、检出 0，属 #3 的检测规则缺口（缺陷记录 D11）。
8. **小时表有 47 个小时缺口**（expected 15,120 / actual 15,073，`rowOffsetMlSafe=false`）：ML 侧**不得**直接按行偏移做特征，必须先整点网格对齐（见 `part2/ml/MODEL_REPORT.md` §12）。前端与 ADS 侧不受影响。
9. **老师"Qt 端加大屏入口"这条要求不存在**：2026-09-15 澄清为口误，主机浏览器直接开大屏即可；`docs/management/part2-design-review.md` 的 R3 已作废，**不要再往 Qt 端加入口**。
