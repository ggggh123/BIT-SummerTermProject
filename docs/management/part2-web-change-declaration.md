...
7. **ADS 质量表 R02/R04 检出 0**：注入各 11,200 条、检出 0。**两者均已归因并修复**：R02 是**注入端 bug**（生成器注入重复行时跳过业务主键，检测端按契约 `primary_key` 分组永远命中不到；#2 拍板「按业务主键判重」并改生成器）；R04 是 **ADS 侧独立复算的计数口径缺口**（此前仅「不可解析」才计；PRL 侧代码早已按契约判定，集成机实跑 `Q4 TP=11,200 / 召回 1.0`），已修复并补测试（缺陷 D11/D15）。**旧批次为修复前产物故为 0，重跑后两项均转非 0**。
8. **区政府页「平均等待」恒为 0 —— 是数据特征，不是 bug（2026-09-15 重跑后的最终结论）**：① 曾有一处技术缺陷——DWD 时间列是 STRING，Spark 上直接 `UNIX_TIMESTAMP(字符串)` 返回 `null`，相减取平均静默变 0，SQL 已改为显式 `CAST`；② **修完 SQL 后仍为 0**，因为生成器对 `reserved_at` 与 `started_at` 写入**同一时间戳**（`part2/scml/data_generator/generator.py:486-487`），重跑批次实测 **110,696 / 110,696 条已完成订单两者相等**。要让该值非 0 需改生成器并重跑（后续批次议题）。**不要再表述为 SQL bug**（缺陷记录 D14）。
9. ~~**「保持当前批次」还是「在演示机重跑整链路」尚未拍板**~~ → **已拍板（2026-09-15 晚，#2）**：**在演示机重跑整链路**，并同步把演示机由 `TimeMachine` 迁至 `niyujun01`（192.168.59.128）；ML 按 #5 的四步机制在演示机**代 #5 重跑**（不取 #5 的 `forecast.db`，避免异源批次）。见 `part2-defect-log.md` §2.1。
10. **小时表有 52 个小时缺口**（新批次实测 expected 17,280 / actual 17,228，`rowOffsetMlSafe=false`）：ML 侧**不得**直接按行偏移做特征，必须先整点网格对齐（训练时补齐至 17,280 行，见 `part2/ml/MODEL_REPORT.md` §12）。前端与 ADS 侧不受影响。
