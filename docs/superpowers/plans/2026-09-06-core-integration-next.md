...
本文件是团队集成与验收计划，不替代各修复 PR 的细化实现设计。它给出已定位文件、精确输入、验收断言和串联顺序。用户已明确授权本机补交跨模块修复；下述负责人保留模块归属，实施来源及本地候选记录在修复报告中，不假定队友已经合并或签署验收。
...
- [x] 先让回归复现报告的红灯：黄金库用户预约 `INVALID_RESPONSE`、新用户登录 `INVALID_RESPONSE`。检查 `registeredAt/reservedAt/startedAt/endedAt/updatedAt/acceptedAt`，在本机时区和 UTC 进程环境下都要求显式 `+08:00`。
- [x] 修复后使用实际本机 `UserApi/TcpJsonClient` 再连接服务端，要求新老用户均正常登录、预约成功收到 `chargeOrderChanged`，不产生 `uncertain=true`。检查真实回包，不能只修测试 fixture。
- [x] 新订单按 150 分/kWh 站点注入两次遥测，增量依次为 0.25 和 0.50。停止前后断言均为 `energyKwh=0.75`、`amountFen=113`；`elapsedSec` 来自开始/停止时间，不是固定 0/3600。
- [x] 同一 `requestId` 重发充值 100 分：余额只增加 100；同一遥测请求只累加一次；同一结算请求只扣款一次；每组重复请求取得相同首次 ACK，设备 cursor 不先于重放检查。
- [x] 注入 SQL 失败/锁冲突：业务不得部分成功后返回 OK；余额、订单、桩状态和已保存响应必须一致。失败必须使用合同错误域，不直接返回 SQL 文本。
- [x] 单项红灯转绿后，再运行全部相关回归，不只运行新用例。
...
**2026-09-06 执行记录：** 服务端 `cfa00e7+dcd7d5a` 独立审查通过；组合生产基线 `2318d14` 的全新 CTest 24/24，以及 UTC/上海×新/老账户实际 Qt UserApi 四组完整诊断通过。见 `docs/test/core-fixes-2026-09-06.md`。事务修复不等于合同 DatabaseWorker 线程架构已完成。
...
- [x] 新增线上编码回归：两个时间分别为 `.001+08:00` 和 `.002+08:00` 的设备事件，经过实际发送/解码后仍严格递增，不能都变成整秒。覆盖遥测、故障、恢复及断线队列重发。
- [ ] 验证初始暂停 → 运行 → 暂停的实际服务端状态；默认空 token 必须识别为鉴权失败，不将“已连接”误记为业务接入成功。
- [x] 验证用户预约/开始/停止、管理重启之后的权威桩状态同步。首轮临时使用手动刷新时写入记录；最终演示流程中的自动/显式刷新必须明确且稳定。
- [x] 修正或明确 GUI 参数入口：现在传 `--db` 会进入无界面模式。必须能启动使用指定黄金库运行副本的管理窗口；不并行启动第二个占用 9100 的服务端，不让管理窗口显示另一数据库。
- [x] 校验核心黄金库哈希与 manifest；重建时输出到新目录，分别保留 core/demo 的 manifest，不覆盖旧成果。运行副本启动前校验，启动迁移后不再要求运行文件哈希等于封存文件。
- [x] 复验管理端月营收按自然月统计，故障/充电/结算后页面取得新的权威数据。若没有告警逻辑，界面和答辩不得宣称已实现实时告警。
...
**2026-09-06 执行记录：** 模拟器/数据 `6e44c18+6a69d0b`、管理端 `9fc9919+237e2b0` 已独立审查通过并组合；管理 Qt 启动/刷新、模拟器网络/窗口与黄金工具回归均通过。实际 SimulatorWindow 对真实服务端的跨模块测试 `99bcecd+c6dda3d` 已补交并审查，验证生产定时器自动同步 charging/重启后 idle；没有将 fake-server 单测冒充该联调。
...
- [x] 以订单 ID 串联证据；停止前后电量和金额连续，结算余额差等于订单金额；历史新增一条完成订单，桩按当前权威状态释放或保持故障，不用人工 SQL 修补。
...
**2026-09-06 部分执行记录：** `core_workflow` 已通过实际 Qt UserApi + SimulatorWindow + 外部真实服务端的正常/故障两条链路（源 `99bcecd+c6dda3d`，本地 `1e349a6+9faad1f`），费用/余额/管理营收、自动同步与故障恢复未结算 guard 均有程序化证据。该记录不包含腾讯在线调用、人工连续三窗口操作或第二批 UI，故不勾选完整主线和所有异常场景。
...
Tasks 1–3 的代码补丁已审查并汇入本地候选，组合构建/既有回归通过。继续 Task 4 的实际模拟器组合链路与异常路径；专属 DatabaseWorker、合同在线 reset、第二批 UI 和人工/地图彩排仍需处理。不因前三个 P0 已修复就宣称项目 GO，不提前发布共享分支。
