...
2. 让 README、项目计划、需求矩阵、风险表、接口合同和发布闸门使用同一口径。
...
以 `origin/main` 最新的 `需求矩阵-第1组-王浩恩.xlsx` 为基础，不恢复旧文件名，也不覆盖队友新版本。

- 保留全部现有行号与两个工作表；
- Web 与 ML 行不删除，状态改为 `N/A` 或等价的“退出核心验收，成果保留”；
- 对相关详细说明增加“可选参考成果”口径；
- 解除服务端营收、站点管理、数据库构建等核心行对 Web/ML 的验收依赖；
- 按只读审计证据更新实际进度，不把未联调能力标记为完成；
- 保持现有版式、合并单元格、列宽、字体和日期格式；
- 导出后重新渲染并扫描公式错误。
...
- 吸收此后 `origin/main` 中队友更新的需求矩阵；
- 只修改文档、HTML、需求矩阵以及范围相关的环境脚本；
- 不修改业务实现；
...
| 需求矩阵覆盖队友新版本 | 先吸收 `origin/main` 最新工作簿，以其文件名和版式为基准 |
