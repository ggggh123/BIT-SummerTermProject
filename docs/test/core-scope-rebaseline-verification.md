...
本记录证明上述提交上的范围文档、环境 profile、数据库测试、页面合同测试以及当前 CMake/CTest 构建图通过了本页所列命令；**不证明核心真实端到端闭环已经完成或 core 已 GO**。当前服务端 P0（`R9`–`R11`）、模拟器/真实服务端集成证据限制（`R12`）、模拟器具体 P1（`R13`、`R14`）、用户端会话 P1（`R15`）和同一提交两次彩排（`R7`）仍未关闭，具体见本文末尾“已知未修复事项”。
...
- **服务端 P0（`R9`、`R10`、`R11`，阻断 core GO）**：仅有 5/27 action 的实现证据；根 CMake 尚未纳入服务端 target，缺少专属测试；预约→充电→遥测→停止→结算状态机、串行写库以及余额/订单/统计一致性尚未在真实服务端闭合。
- **模拟器两项具体 P1（代码风险）**：
  1. **`R13`**：`SimulatorClient::sendStatus()` 当前无条件发送 `state=running`，不会反映 Pause 或停止状态；状态上报因而可能与模拟器实际运行状态不一致。本记录未产生 running/paused/stopped 针对性载荷断言和真实服务端转移日志，因此风险仍开放。
  2. **`R14`**：`TelemetryEngine::requestFault()` 与 `TelemetryEngine::requestRecovery()` 都使用未推进的 `currentTime_` 写入 `recordedAt`。连续 fault/recovery，或它们与同一时刻事件交错时，时间戳可能不严格递增，不满足共享 device-event cursor 的严格递增约束。本记录未产生同 tick 事件序列回归和 cursor 完整消费日志，因此风险仍开放。
- **模拟器集成证据限制（`R12`）**：状态/遥测/故障事件尚未与真实服务端完成端到端接入证明；双实例/双路径的现场行为也不能由数据库或当前 CTest 推导。该集成限制不能替代 `R13`/`R14`，也不能因本页 13/13 CTest 而自动关闭。
- **用户端会话 guard/race（`R15`）**：会话失效后重登录、快速切换账号或响应乱序时，旧会话回调/迟到响应不得污染新会话、显示旧用户数据或推动页面前进/跳转。本 PR 未修改用户端业务代码，也未产生主动注入旧响应的确定性回归和真实服务端重放日志；页面合同测试、单进程 QtTest 或现有会话测试不能证明风险已关闭。
