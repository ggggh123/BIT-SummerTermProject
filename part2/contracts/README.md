...
4. #2／#4／#5：决定 Q5 超占用小时是隔离还是裁剪、Q9 越界坐标是隔离还是回填，并确认 ML 如何处理小时缺口。
   ...
   **但按「Q2 口径已定、整体策略文件仍为 `DRAFT_PENDING_TEAM_REVIEW`」的现状，本项暂不标记冻结**（#2 决定），待与 Q2 等条目一并走完评审流程。已在本机用带 47 个缺口的真实 DWD 验证通过。
   ...
   → **Q9 越界坐标**：**待 #2 最终确认**。已核实现状为「隔离整站」——#3 的 `record_rules.py:146` 为 `reject`，导致 ODS 的 8 个站点在 DWD 中只剩 7 个（`dim_stations: 7`），该站关联订单等触发 Q7 级联隔离，正式批次订单由 12 万降至 102,332。备选方案「仅置空坐标（保留站点记录、经纬度置 NULL）」可避免该损失；「回填坐标」因无外部权威数据源、等同编造数据而不建议采用。
