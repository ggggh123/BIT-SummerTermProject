...
状态：2026-09-15 正式规模链路已验证。#4 的 manifest、ODS 字段和七表 DWD schema 已适配，真实 YARN 清洗及 DWD→DWS→ADS→ML→Flask/web 链路已通过；质量处置策略仍未经团队冻结。本说明描述清洗实现细节，不修改第一阶段 Qt／数据库代码。正式行数、哈希和回归结果以 [正式规模验证记录](quality-verification.md) 为准；正文中提到的小样本仅为历史夹具说明。
