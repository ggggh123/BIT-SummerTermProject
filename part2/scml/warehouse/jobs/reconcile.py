...
def read_dwd(path: Path, table: str) -> list[dict] | None:
    ...
    try:
        # 集成机的 core-site.xml 把默认文件系统设为 HDFS；绝对路径若不带 file://，
        # Spark 仍会拿它去访问 NameNode。交接包明确位于本机，必须固定 local URI。
...
def main() -> int:
    ...
    hourly_rows = scalar("SELECT COUNT(1) FROM ads_station_hourly")
    hourly_grid = stations_n * 24 * days_n
    quality_meta = dict(connection.execute("SELECT key, value FROM ads_quality_meta"))
    if quality_meta.get("source") == "prl-quality-report":
        coverage = hourly_rows / hourly_grid if hourly_grid else 0.0
        report.check(
            group,
            "PRL 小时表无超额行且完整率 ≥ 95%",
            hourly_rows <= hourly_grid and coverage >= 0.95,
            f"{hourly_rows}/{hourly_grid}，完整率={coverage:.2%}",
        )
    else:
        report.close(group, "小时表行数 == 站点数 × 24 × 天数",
                     hourly_rows, hourly_grid, COUNT_TOL)
    ...
    batch_summary = connection.execute(
        "SELECT model_version, is_baseline, note FROM ads_forecast_batch LIMIT 1"
    ).fetchone()
    metrics_summary = connection.execute(
        "SELECT COUNT(1), AVG(wape), AVG(baseline_wape) FROM ads_forecast_metric"
    ).fetchone()
    batch_flag = int(batch_summary["is_baseline"]) if batch_summary else -1
    baseline_note_ok = batch_flag != 1 or "降级" in str(batch_summary["note"] or "")
    report.check(
        group,
        "预测批次存在且基线/ML 身份如实标注",
        batch_summary is not None and batch_flag in (0, 1) and baseline_note_ok,
        "缺少批次，is_baseline 非 0/1，或基线未注明降级",
    )
    if metrics_summary[0] and metrics_summary[1] is not None and metrics_summary[2] is not None:
        label = "基线预测" if batch_flag == 1 else "ML 预测"
        info_lines.append(
            f"{label} WAPE 均值 {metrics_summary[1]:.2f}% vs 对照基线 "
            f"{metrics_summary[2]:.2f}%"
        )
